...
async def signup(request: SignupRequest):
    ...
    try:
        ...
        # Create new player - STORE PLAIN TEXT (for MVP only!)
        player_id = str(uuid.uuid4())
        
        new_player = {
            "id": player_id,
            "username": username,
            "password": password,  # ⚠️ Plain text - CHANGE THIS LATER!
            "coins": 100,
            "level": 1,
            "created_at": "now()"
        }
...
async def login(request: LoginRequest):
    ...
    try:
        ...
        # ⚠️ SIMPLE PASSWORD CHECK - Plain text comparison
        stored_password = player.get("password", "")
        if password != stored_password:  # Changed from password[:50]
            return PlayerResponse(
                success=False, 
